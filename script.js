...
/* script.js - complete interactive behavior */
...
const GITHUB_URL = "https://github.com/goat82447-ops"; // update if different
const RESUME_FILE = "assets/Krishna-Kumar-Bandoju-Resume.pdf"; // make sure file is at assets/
...
/* Project case study content (simple templates) */
const PROJECT_CASES = {
  dynamics: {
  ...
  lowcode: {
    ...
    overview: "Low-code platform to build modular enterprise applications.",
    responsibilities: [
      "ASP.NET Core backend and Angular frontend development.",
      "API versioning, DI, middleware and logging using Serilog.",
      "BDD automation and Azure-based CI/CD pipelines."
    ],
    tech: [ "ASP.NET Core", "Angular", "BDD", "Azure CI/CD"],
    challenges: ["Ensuring extensibility and plugin compatibility."],
    solutions: ["Clear extension points and strict contract tests."],
    impact: ["Enabled faster client delivery cycles."]
  },
  ...
  atwork: {
    ...
    client: "AmpleLogic",
    role: "Senior Software Engineer",
    overview: "Workforce management and reporting system.",
    responsibilities: ["ASP.NET MVC backend, Angular frontend, SQL Server queries", "Performance and logging improvements"],
    tech: ["ASP.NET MVC", "Angular", "SQL Server", "LINQ"],
    challenges: ["Large reporting datasets."],
    solutions: ["Query optimization and indexed reporting tables."],
    impact: ["Better report performance and UX."]
  },
  ...
  elms: {
    ...
    client: "AmpleLogic",
    role: "Senior Software Engineer",
    overview: "Enterprise learning management with approval workflows.",
    responsibilities: ["Frontend validation, backend workflows, reporting"],
    tech: ["ASP.NET MVC", "Angular", "SQL Server"],
    challenges: ["Complex approval chains"],
    solutions: ["Workflow engine improvements and UI validations"],
    impact: ["Reduced approval errors and improved traceability"]
  },

  hrbenefits: {
    title: "HR Benefits Management System",
    client: "Shell Info Technologies",
    role: "Software Engineer",
    overview: "Employee benefits and plan management solution.",
    responsibilities: ["Feature development, NuGet dependency maintenance, operational monitoring"],
    tech: ["ASP.NET", "SQL Server"],
    challenges: ["Keeping dependent packages up to date"],
    solutions: ["Automated dependency checks and monitoring"],
    impact: ["Improved stability and alerts."]
  }
};
...
document.addEventListener("DOMContentLoaded", () => {
  // Wire up top-level links and buttons
  setupTopLinks();
  buildSkills();
  setupNav();
  setupMobileMenu();
...
function setupTopLinks(){
  const linkedinButtons = [$("#linkedinBtn"), $("#linkedinSecondary"), $("#contactLinkedIn"), $("#footerLinkedIn"), $("#footerLinkedIn")].filter(Boolean);
  const githubButtons = [$("#githubBtn"), $("#githubSecondary"), $("#contactGitHub"), $("#footerGitHub")].filter(Boolean);
...
function buildSkills(){
  ...
  container.innerHTML = "";
  Object.keys(SKILLS_DATA).forEach((category, idx)=>{
...
/* Nav: smooth scrolling & active class on click */
function setupNav(){
...
/* Skill accordion behavior - only one open at a time */
function setupSkillAccordion(){
  let openCard = null;
  const cards = $$(".skill-card");
  cards.forEach(card=>{
    card.addEventListener("click", (e)=>{
      // allow click on inner buttons to not toggle, but we want card toggle
      const panel = card.querySelector(".skill-panel");
...
/* Project modal: open/close and populate */
  ...
  return `
    <div class="modal-body">
      ...
      <div style="margin-top:1rem;display:flex;gap:.6rem">
        <button class="btn btn-primary" id="modalCloseAction">Close</button>
      </div>
    </div>
  `;
}

/* Modal close handlers (escape, backdrop, button) */
function setupModalCloseHandlers(){
  ...
  // delegate close button inside modal content (Close)
  ...
  // clicking outside modal-panel also closes modal (backdrop covers entire area)
}
...
function closeModal(){
  ...
  // clear content after a short delay
...
/* Wire resume download and email links in header/footer */
function wireResumeAndEmail(){
  // set email in anchors
  ...
  emailAnchors.forEach(a=>{
    if(a.id === "emailBtn" || a.id === "footerEmail"){
      ...
      a.addEventListener("click", ()=>{}); // let default mail client open
    } else {
      a.setAttribute("href", `mailto:${email}`);
    }
  });

  // set resume links set earlier via setupTopLinks - ensure fallback
  // set footer link text urls
  ["contactLinkedIn","contactGitHub","footerLinkedIn","footerGitHub"].forEach(id=>{
  ...
  // set header download anchors (if any left)
  const downloadBtns = [$("#downloadResumeBtn"), $("#downloadResumeHero"), $("#downloadResumeFooter")].filter(Boolean);
  downloadBtns.forEach(b => b.setAttribute("href", RESUME_FILE));
}

/* Small badge floating animation (decorative) */
function animateBadges(){
  const badgeList = $("#badgeList");
  if(!badgeList) return;
  // gently translate badges on a slow loop
